...
export default class ResizableColumns {
  ...
  **/
  createHandles() {
    ...
    this.$tableHeaders.each((i, el) => {
      ...
      if (
        $next.length === 0 ||
        $current.is(SELECTOR_UNRESIZABLE) ||
        $next.is(SELECTOR_UNRESIZABLE)
      ) {
        return;
      }
  ...
  saveColumnWidths() {
    this.$tableHeaders.each((_, el) => {
      ...
      if (this.options.store && !$el.is(SELECTOR_UNRESIZABLE)) {
        this.options.store.set(this.generateColumnId($el), this.parseWidth(el));
      }
    });
  }
  ...
  **/
  restoreColumnWidths() {
    this.$tableHeaders.each((_, el) => {
      ...
      if (this.options.store && !$el.is(SELECTOR_UNRESIZABLE)) {
        let width = this.options.store.get(this.generateColumnId($el));
  ...
  **/
  onPointerDown(event) {
    ...
    let $leftColumn = this.$tableHeaders
      .eq(gripIndex)
      .not(SELECTOR_UNRESIZABLE);
    let $rightColumn = this.$tableHeaders
      .eq(gripIndex + 1)
      .not(SELECTOR_UNRESIZABLE);
    ...
    this.$handleContainer.add(this.$table).addClass(CLASS_TABLE_RESIZING);
  ...
  **/
  onPointerMove(event) {
    ...
    let difference =
      ((this.getPointerX(event) - op.startX) / this.$table.width()) * 100;
    if (difference === 0) {
      return;
    }
    ...
    if (difference > 0) {
      widthLeft = this.constrainWidth(
        op.widths.left + (op.widths.right - op.newWidths.right),
      );
      widthRight = this.constrainWidth(op.widths.right - difference);
    } else if (difference < 0) {
      widthLeft = this.constrainWidth(op.widths.left + difference);
      widthRight = this.constrainWidth(
        op.widths.right + (op.widths.left - op.newWidths.left),
      );
    }
    ...
    return this.triggerEvent(
      EVENT_RESIZE,
      [op.$leftColumn, op.$rightColumn, widthLeft, widthRight],
      event,
    );
  }
  ...
  **/
  onPointerUp(event) {
    ...
    this.unbindEvents(this.$ownerDocument, [
      'mouseup',
      'touchend',
      'mousemove',
      'touchmove',
    ...
    this.$handleContainer.add(this.$table).removeClass(CLASS_TABLE_RESIZING);
    ...
    return this.triggerEvent(
      EVENT_RESIZE_STOP,
      [op.$leftColumn, op.$rightColumn, op.newWidths.left, op.newWidths.right],
      event,
    );
  }
  ...
  **/
  destroy() {
    ...
    this.unbindEvents(
      this.$window.add(this.$ownerDocument).add(this.$table).add($handles),
    );
